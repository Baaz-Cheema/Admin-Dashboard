...
export default function ArticleItem({ data }) {
    ...
    return (
        <div className="border border-zinc-700 rounded-lg font-roboto flex flex-col">
            {isLoading && <Skeleton className={'h-[60%] rounded-lg'} />}
            <a href={data.link} className={`${isLoading?'h-0':'h-[60%]'}`} style={{ display: isLoading ? 'none' : 'block' }}>
